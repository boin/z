Formatter = {
	...
	format: function(doc) {
		...
		var fn = 'style_' + this.isSuppoted(doc);

		doc.selectAll();
		doc.cut();
		var content = (this[fn])(UltraEdit.clipboardContent, {
			indent_size: 1,
			indent_char: '\t'
		});
		doc.write(content);
	...
	isSuppoted: function(doc) {
		var supportedTypes = this.supportedTypes,
		i, fn;
		for (fn in supportedTypes) {
			UltraEdit.outputWindow.write("checking:" + fn);
			var exts = supportedTypes[fn];
			for (i in exts) {
				UltraEdit.outputWindow.write("checking:" + fn + "[" + i + "] : " + exts[i]);
				if (doc.isExt(exts[i])) {
					UltraEdit.outputWindow.write("found match ext:" + fn);
					return fn;
				}
			}
			UltraEdit.outputWindow.write("nothing found in : " + fn);
		}
		throw new Error('sorry, this file is not supported');
	},
...
*/
	style_js: function(js_source_text, options) {
...
*/
	style_html: function(html_source, options) {
...
Formatter.format(UltraEdit.activeDocument);